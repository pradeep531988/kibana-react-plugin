    ...
    return projects
      .map((project) => {
        const projectDetailWithLinks = (
          <EuiFlexItem style={{ minHeight: 200 }}  className="projectPanel" key={project.id} id={project.id}>
            <ProjectDetails
        ...
        { project.length > 0 ? (
          <div>
            <EuiFlexGroup gutterSize="s" className="applicationMain euiPanel euiPanel--paddingMedium">
              <EuiFlexItem columns={1}>
                <EuiTitle  size="s" className="subdued" style={{ paddingBottom: 10 }}>
                  <h4>
