...
function populateDropDown(location) {
  ...
  const hashChecker = ['#/discover', '#/visualize', '/dashboards']; //Dropdown allowed location
  ...
  checkElement('.kuiLocalMenu')
    .then((element) => {
      if (dropDownSet === false) {
        const applicationJson = getApplicationList('https://3a11475c-2b9c-4128-b144-586ee8dca40b.mock.pstmn.io/scroll');
        let dropDownValues = [];
